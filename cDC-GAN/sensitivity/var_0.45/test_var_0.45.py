...
for t_p in time_point:
    ...
    k_test = []
    for m in range(60):
        k_data = np.array(pd.read_csv('D:/surrogate_model/program/cdcgan_final/all_data/sensitivity/var_0.45/k_data/第' + str(m+star_num) + '个场.txt', header=None, sep=' '))
        k_data = k_data
        max = np.max(k_data)
        min = np.min(k_data)
        k_normal = (k_data-min)/(max-min)   # 归一化数据，分布在0-1之间
        with open('D:/surrogate_model/program/cdcgan_final/all_data/sensitivity/var_0.45/ion/'+t_p+'d/no_'+str(m + star_num)+'_'+t_p+'天化学结果.pkl', 'rb') as f:
    ...
    modelGenerator.cuda()
    # 加载 train.py 里训练好的模型
    modelGenerator.load_state_dict(torch.load('./result/152d/newh_G_99model.pt'))  # 152d use
    # modelGenerator.load_state_dict(torch.load('./result/300d/newh_G_99model.pt'))   # 300d use
    # modelGenerator.load_state_dict(torch.load('./result/650d/newh_G_99model.pt'))  # 650d use
