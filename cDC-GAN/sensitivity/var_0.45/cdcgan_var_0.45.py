...
class Generator(nn.Module):
    ...
    def forward(self, well, label):
        # label = label[0,0,:,:].reshape(1, 64**2)
        # label = self.label(label)
        # # label = linear.change(label.cpu()).cuda()
        # z = torch.concat([z, label], dim=1)
        # out = self.l1(z)              # 这一步放到cpu中去做
        # well = self.bn(well)  # 归一化处理
...
for t_p in time_point:
    for m in range(200):
        k_data = np.array(pd.read_csv('D:/surrogate_model/program/cdcgan_final/all_data/sensitivity/var_0.45/k_data/第' + str(m) + '个场.txt', header=None, sep=' '))
        k_data = k_data
        max = np.max(k_data)
        min = np.min(k_data)
        k_normal = (k_data-min)/(max-min)   # 归一化数据，分布在0-1之间
        with open('D:/surrogate_model/program/cdcgan_final/all_data/sensitivity/var_0.45/ion/'+t_p+'d/no_'+str(m)+'_'+t_p+'天化学结果.pkl', 'rb') as f:
...
for epoch in range(opt.n_epochs):
    for i, imgs in enumerate(x_train):
        ...
        fake_loss = adversarial_loss(discriminator(wel, gen_imgs.detach(), label), fake)  # 判别器判别假图片是假的的损                    失
        ...
        # 5天
        if (epoch+1) % 25 == 0 and i == 95:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/15d/' + str(epoch) + '_95.png')
        if (epoch+1) % 25 == 0 and i == 96:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/15d/' + str(epoch) + '_96.png')
        if (epoch+1) % 25 == 0 and i == 199:
            torch.save(generator.state_dict(), './result/15d/newh_G_{}model.pt'.format(epoch))  # 模型保存
            # torch.save(discriminator.state_dict(), "./imgs_all/102/D_{}model.pt".format(epoch))  # 模型保存
        a = 0
        # 10天
        if (epoch +1) % 25 == 0 and i == 295:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/102d/' + str(epoch) + '_295.png')
        if (epoch + 1) % 25 == 0 and i == 296:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/102d/' + str(epoch) + '_296.png')
        if (epoch + 1) % 25 == 0 and i == 399:
            torch.save(generator.state_dict(), './result/102d/newh_G_{}model.pt'.format(epoch))  # 模型保存
        aa = 0
        # 15天
        if (epoch +1) % 25 == 0 and i == 495:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/152d/' + str(epoch) + '_495.png')
        if (epoch +1) % 25 == 0 and i == 496:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/152d/' + str(epoch) + '_496.png')
        if (epoch + 1) % 25 == 0 and i == 599:
            torch.save(generator.state_dict(), './result/152d/newh_G_{}model.pt'.format(epoch))  # 模型保存

        # 30天
        if (epoch +1) % 25 == 0 and i == 695:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/201d/' + str(epoch) + '_95.png')
        if (epoch + 1) % 25 == 0 and i == 696:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/201d/' + str(epoch) + '_96.png')
        if (epoch + 1) % 25 == 0 and i == 799:
            torch.save(generator.state_dict(), './result/201d/newh_G_{}model.pt'.format(epoch))  # 模型保存
                # torch.save(discriminator.state_dict(), "./imgs_all/102/D_{}model.pt".format(epoch))  # 模型保存
            a = 0
        # 60天
        if (epoch +1) % 25 == 0 and i == 895:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/251d/' + str(epoch) + '_295.png')
        if (epoch + 1) % 25 == 0 and i == 896:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/251d/' + str(epoch) + '_296.png')
        if (epoch + 1) % 25 == 0 and i == 999:
            torch.save(generator.state_dict(), './result/251d/newh_G_{}model.pt'.format(epoch))  # 模型保存
        aa = 0
        # 102天
        if (epoch+1) % 25 == 0 and i == 1095:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/300d/' + str(epoch) + '_495.png')
        if (epoch + 1) % 25 == 0 and i == 1096:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/300d/' + str(epoch) + '_496.png')
        if (epoch + 1) % 25 == 0 and i == 1199:
            torch.save(generator.state_dict(), './result/300d/newh_G_{}model.pt'.format(epoch))  # 模型保存

        # 152天
        if (epoch+1) % 25 == 0 and i == 1295:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/350d/' + str(epoch) + '_495.png')
        if (epoch + 1) % 25 == 0 and i == 1296:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/350d/' + str(epoch) + '_496.png')
        if (epoch + 1) % 25 == 0 and i == 1399:
            torch.save(generator.state_dict(), './result/350d/newh_G_{}model.pt'.format(epoch))  # 模型保存
        # 201天
        if (epoch +1) % 25 == 0 and i == 1495:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/450d/' + str(epoch) + '_95.png')
        if (epoch + 1) % 25 == 0 and i == 1496:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/450d/' + str(epoch) + '_96.png')
        if (epoch + 1) % 25 == 0 and i == 1599:
            torch.save(generator.state_dict(), './result/450d/newh_G_{}model.pt'.format(epoch))  # 模型保存
        a = 0
        # 251天
        if (epoch +1) % 25 == 0 and i == 1695:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/551d/' + str(epoch) + '_295.png')
        if (epoch + 1) % 25 == 0 and i == 1696:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/551d/' + str(epoch) + '_296.png')
        if (epoch + 1) % 25 == 0 and i == 1799:
            torch.save(generator.state_dict(), './result/551d/newh_G_{}model.pt'.format(epoch))  # 模型保存
            aa = 0
        # 300天
        if (epoch+1) % 25 == 0 and i == 1895:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/650d/' + str(epoch) + '_495.png')
        if (epoch + 1) % 25 == 0 and i == 1896:
            plt.imshow(torch.Tensor.detach(gen_imgs[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/650d/' + str(epoch) + '_496.png')
