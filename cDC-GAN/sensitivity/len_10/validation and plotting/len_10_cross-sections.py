...
for t_p in time_point:
    ...
    for i in range(60):
        if i not in ti_chu_152:
            with open('D:/surrogate_model/program/cdcgan_final/sensitivity/len_10/result/' + t_p + 'd/test_data/real_ion{}.plk'.format(i), 'rb') as f:
                real = pickle.load(f)  # 保存铀浓度数据
            with open('D:/surrogate_model/program/cdcgan_final/sensitivity/len_10/result/' + t_p + 'd/test_data/fake_ion{}.plk'.format(i), 'rb') as f:
                fake = pickle.load(f)
...
# plt.show()
plt.savefig('./len_10.png', dpi=500)
plt.close()
