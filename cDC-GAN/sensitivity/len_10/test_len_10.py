...
for t_p in time_point:
    ...
    k_test = []
    for m in range(60):
        k_data = np.array(pd.read_csv('D:/surrogate_model/program/cdcgan_final/all_data/sensitivity/len_10/k_data/第' + str(m+star_num) + '个场.txt', header=None, sep=' '))
        k_data = k_data
        max = np.max(k_data)
        min = np.min(k_data)
        k_normal = (k_data-min)/(max-min)   # 归一化数据，分布在0-1之间
        with open('D:/surrogate_model/program/cdcgan_final/all_data/sensitivity/len_10/ion/'+t_p+'d/no_'+str(m + star_num)+'_'+t_p+'天化学结果.pkl', 'rb') as f:
    ...
    # modelGenerator.load_state_dict(torch.load('./result/350d/newh_G_99model.pt'))   # 300d use
