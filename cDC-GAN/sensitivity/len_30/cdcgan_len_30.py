...
for t_p in time_point:
    for m in range(200):
        k_data = np.array(pd.read_csv('.../all_data/sensitivity/len_30/k_data/第' + str(m) + '个场.txt', header=None, sep=' '))
        k_data = k_data
        max = np.max(k_data)
        min = np.min(k_data)
        k_normal = (k_data-min)/(max-min)   # 归一化数据，分布在0-1之间
        with open('.../all_data/sensitivity/len_30/ion/'+t_p+'d/no_'+str(m)+'_'+t_p+'天化学结果.pkl', 'rb') as f:
