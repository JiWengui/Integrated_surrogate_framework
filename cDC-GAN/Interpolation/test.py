...
for t_p in time_point:
    ...
    k_test = []
    for m in range(260):
        k_data = np.array(pd.read_csv('D:/surrogate_model/program/dcgan/cDCGAN/cdcgan-again/64x64_final/data/k_data/第' + str(m+star_num) + '个场.txt', header=None, sep=' '))
        k_data = k_data
        max = np.max(k_data)
        min = np.min(k_data)
    ...
    modelGenerator.cuda()
    # 加载 train.py 里训练好的模型
    # modelGenerator.load_state_dict(torch.load('./result/251d/newh_G_49model.pt'))#201use  旧的。不用因为他的ssim值比551天的还小
    modelGenerator.load_state_dict(torch.load('./result/251d/newh_G_99model.pt'))#201use  新的
    # modelGenerator.load_state_dict(torch.load('./result/300d/newh_G_49model.pt'))   # 350 use
    # modelGenerator.load_state_dict(torch.load('./result/450d/newh_G_99model.pt'))  # 551 use
    modelGenerator.load_state_dict(torch.load('./result/450d/newh_G_99model.pt'))  # 551 use  新的
    ...
    for i in range(260):
        ...
        plt.imshow(torch.Tensor.detach(fake_img[0, 0, :, :]).cpu().numpy())  # my
        plt.savefig('./result/'+t_p+'d/test/' + str(i) + '.png')  # 650天

        plt.imshow(x_test[i])  # my
