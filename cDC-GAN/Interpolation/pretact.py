...
for t_p in time_point:
    ...
    modelGenerator.cuda()
    # 加载 train.py 里训练好的模型
    modelGenerator.load_state_dict(torch.load('./result/450d/G_99model.pt'))
    ...
    for i in range(60):
        ...
        if idx == 0:
            ...
            plt.savefig('./result/650d/test/' + str(i) + '_551.png')  # 650天
            plt.imshow(x_test[i])  # my
            plt.savefig('./result/650d/test/真' + str(i) + '_551.png')
        if idx == 1:
            plt.imshow(torch.Tensor.detach(fake_img[0, 0, :, :]).cpu().numpy())  # my
            plt.savefig('./result/650d/test/' + str(i) + '_650.png')  # 650天
            plt.imshow(x_test[i])  # my
            plt.savefig('./result/650d/test/真' + str(i) + '_650.png')
