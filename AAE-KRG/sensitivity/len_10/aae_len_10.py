...
for m in range(260):
    ...
    max = np.max(k_data)
    min = np.min(k_data)
    k_normal = (k_data-min)/(max-min)   # Normalize to [0,1]
...
for epoch in range(args.n_epochs):
    ...
    if (epoch + 1) % 100 == 0:
        torch.save(encoder.state_dict(), './result/encoder_{}model.pt'.format(epoch))  # save encoder
        torch.save(decoder.state_dict(), "./result/decoder_{}model.pt".format(epoch))  # save decoder