...
ens_no = 260
start_num = 0
path_txt_9 = '../all_data/well_data/var_0.15/well_9'
...
with open('../all_data/krg_model/var_0.15/KRG_wel9_model.pkl', 'rb') as f:
    sm = pickle.load(f)
all_laten = []
for i in range(250):
...
# Using the trained Kriging model to predict leaching concentration over time
...
# load KRG of C2
with open('../all_data/krg_model/var_0.15/KRG_wel10_model.pkl', 'rb') as f:
    sm = pickle.load(f)
all_laten = []
for i in range(250):
    with open('../all_data/k_laten/var_0.15/no_{}'.format(i), 'rb') as f:
...
'''
# Calculate the R-squared and WMAPE of C1
ti_r2 = []
all_laten = []
for i in range(250):
    if i not in ti_r2:
...
# Calculate the R-squared and WMAPE of C2
with open('../all_data/krg_model/var_0.15/KRG_wel10_model.pkl', 'rb') as f:
    sm = pickle.load(f)
...
ax0.text(73,0.0001435,'C 1', fontsize=12, color='black')
# # subfiguer 2
...
ax0.text(0.899,14.2,'C 1', fontsize=12, color='black')
# # subfiguer 3
ax1 = fig.add_axes([.61, 0.72, .25, .23])
ax1.hist(WMAPE_9, 50)
ax1.text(0.047,7.62,'C 1', fontsize=12, color='black')
...
ax0.set_xlabel('days', fontsize=13, labelpad=0)
ax0.set_ylabel('U (mol/L)', fontsize=11,labelpad=0)
ax0.text(73,0.000398,'C 2', fontsize=12, color='black')
# subfiguer 5
...
ax0.set_ylabel('Frequency', fontsize=11, labelpad=-4)
ax0.set_yticks([0,3,6,9,12,15])
ax0.text(0.9765,14.33,'C 2', fontsize=12, color='black')
# subfiguer 6
ax1 = fig.add_axes([.61, 0.43, .25, .23])
ax1.hist(WMAPE_10, 50)
ax1.text(0.0335,7.68,'C 2', fontsize=12, color='black')
ax1.set_ylabel('Frequency', fontsize=11, labelpad=0)
ax1.set_xlabel('WMAPE', fontsize=11)

plt.savefig('./figuer_var0.15.png', dpi=500)
# plt.show()
