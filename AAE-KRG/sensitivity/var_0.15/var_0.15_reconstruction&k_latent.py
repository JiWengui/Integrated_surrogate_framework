...
for m in range(260):
    if m not in ti_chu:
        k_data = np.array(pd.read_csv('../all_data/k_pri_data/var_0.15/第' + str(m) + '个场.txt',header=None, sep=' '))
        max = np.max(k_data)
        min = np.min(k_data)
...
encoder.load_state_dict(torch.load('./result/encoder_1999model.pt'))    # 原是4999
...
for i in range(260):
    ...
    with open('../all_data/k_laten/var_0.15/no_{}'.format(i), 'wb') as f:
        pickle.dump(laten, f)
